...
func (c *couponDatabase) FindAllUserCouponsByUserID(ctx context.Context, userID uint) ([]res.ResUserCoupon, error) {
	...
	query := `SELECT uc.id, uc.coupon_code, c.coupon_name, c.description, c.image, uc.expire_date 
...
func (c *couponDatabase) UpdateUserCoupon(ctx context.Context, userCoupon domain.UserCoupon) error {
	query := `UPDATE user_coupons SET discount_price = $1, used = $2, last_applied = $3`
	if c.DB.Exec(query, userCoupon.Discount, userCoupon.Used, userCoupon.LastApplied).Error != nil {
